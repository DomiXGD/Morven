...
export function AboutSection() {
  return (
    <div className="gsap-section-content flex min-h-screen items-center justify-center overflow-hidden bg-[var(--surface-low)]">
      <div className="site-shell grid grid-cols-1 items-center gap-10 px-0 py-[clamp(3.5rem,8vh,7.5rem)] md:gap-14 lg:grid-cols-2 lg:gap-20">
        <div className="gsap-animate-in relative order-2 lg:order-1">
          <div className="absolute -inset-10 rounded-full bg-[rgba(211,187,255,0.2)] blur-[120px]" />
          <Image
            className="relative z-10 w-full rounded-2xl border border-white/10 shadow-2xl grayscale transition-all duration-700 hover:grayscale-0"
        ...
        <div className="order-1 space-y-8 lg:order-2">
          <span className="gsap-animate-in brand-mono text-sm uppercase tracking-widest text-[var(--secondary)]">Nuestra Visión</span>
          ...
          <p className="gsap-animate-in text-base leading-7 text-[var(--muted)] sm:text-lg sm:leading-relaxed">
          ...
          <ul className="space-y-4">
            {aboutPoints.map((point) => (
              <li key={point.title} className="gsap-animate-in flex items-start gap-4">
